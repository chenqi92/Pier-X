...
| 4 | **Inter for UI, JetBrains Mono for code** | 两个字体，都是开源。**Inter 510 是签名权重**。Mono 用于终端、代码、路径、IP、端口 |
| 5 | **Density over spectacle** | 这是 IDE 级工具，不是营销页。13px 基础文字、4px 间距栅格、不要巨型标题、不要装饰渐变 |
...
强调色（**全系统唯一彩色**）
─────────────────────────────────────────────
accent.primary     #3574f0   IntelliJ 蓝
...
```
UI:       Inter Variable    (开源)
            必须开启 OpenType: "cv01", "ss03"
Mono:     JetBrains Mono    (开源，IDE 标志性)
Mono Win: Cascadia Code     (Windows fallback)
```

> 在 Pier-X 中**捆绑这两个字体**作为资源（不依赖系统字体）。Inter 和 JetBrains Mono 均为 OFL 协议，可商用。

### 3.2 类型阶梯

**注意：IDE 工具的基础 UI 文字是 13px，不是营销页的 16px。** 这是关键区别。

| Role | Font | Size | Weight | Line Height | Tracking | 用途 |
|---|---|---|---|---|---|---|
| Display | Inter | 32px | 510 | 1.20 | -0.704px | 欢迎页、空状态大字 |
| H1 | Inter | 24px | 510 | 1.30 | -0.4px | 设置页主标题 |
| H2 | Inter | 20px | 510 | 1.35 | -0.2px | 对话框标题 |
| H3 | Inter | 16px | 510 | 1.40 | -0.1px | 卡片标题 |
| Body Large | Inter | 14px | 400 | 1.50 | 0 | 主要阅读文本 |
| **Body** | **Inter** | **13px** | **400** | **1.45** | **0** | **默认 UI 文字（最常用）** |
| Body Emphasis | Inter | 13px | 510 | 1.45 | 0 | 强调标签、菜单项 |
| UI Label | Inter | 13px | 510 | 1.0 | 0 | 按钮、tab、工具栏 |
| Caption | Inter | 12px | 510 | 1.40 | 0 | 状态栏、metadata |
| Small | Inter | 11px | 400 | 1.40 | 0 | 提示、tooltip |
| Mono Code | JetBrains Mono | 13px | 400 | 1.50 | 0 | 终端、代码、SQL |
| Mono Small | JetBrains Mono | 12px | 400 | 1.45 | 0 | 内联代码、路径 |

### 3.3 字体规则

- **510 是签名权重**。所有 UI 标签、按钮、菜单项默认 510。Body 阅读文本用 400。强调用 590（最大）。**禁止使用 700 (bold)** —— Pier-X 的最重权重是 590。
- **大字号用紧凑字距**（-0.1 to -0.7px），≤14px 用 normal 字距。
- **必须开启 OpenType `cv01, ss03`**。这是 Inter 的几何变体，让字体更工程化。少了这个就是普通 Inter，不是 Pier-X 的 Inter。
