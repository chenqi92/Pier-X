...
tokens-file: pier-ui-tauri/src/styles/tokens.css
atoms-file: pier-ui-tauri/src/styles/atoms.css
prototype-style: pier-ui-tauri/src/styles/pier-x.css
---
...
> **React 实现**：`pier-ui-tauri/src/`，tokens 在 `src/styles/tokens.css`，共享类在 `src/styles/atoms.css` / `src/styles/pier-x.css`。
...
- **字体文件**：`pier-ui-tauri/src/styles/fonts.css` 引用本地 IBM Plex；离线时走系统 fallback。
...
所有 panel 文件位于 `pier-ui-tauri/src/panels/`。每个 panel 必须：
...
| CSS 变量定义（单一真源） | `pier-ui-tauri/src/styles/tokens.css` |
| 共享原子 class | `pier-ui-tauri/src/styles/atoms.css` |
| 原型迁移类（dialog / diff 等 `.dlg-*` / `.cmdp-*`） | `pier-ui-tauri/src/styles/pier-x.css` |
| Shell chrome 布局 | `pier-ui-tauri/src/styles/shell.css` |
| Git 面板 scoped 样式 | `pier-ui-tauri/src/styles/git-panel.css` |
| 字体 | `pier-ui-tauri/src/styles/fonts.css` |
| 原子 React 组件 | `pier-ui-tauri/src/components/` |
| Shell 组件（TopBar / Sidebar / TabBar / StatusBar / WelcomeView / Dialogs） | `pier-ui-tauri/src/shell/` |
| Panels | `pier-ui-tauri/src/panels/` |
| Zustand stores（UI 状态） | `pier-ui-tauri/src/stores/` |
| Tauri 命令 wrapper（typed invoke） | `pier-ui-tauri/src/lib/` |
| i18n | `pier-ui-tauri/src/i18n/` |
