...
>
> 交付节奏见 [ROADMAP.md](ROADMAP.md)；视觉 token 见 [../.agents/skills/pier-design-system/SKILL.md](../.agents/skills/pier-design-system/SKILL.md)；代码规则见 [../CLAUDE.md](../CLAUDE.md)。
...
```
┌─────────────────────────────────────────────┐
│ pier-ui-tauri/ (React + TypeScript)         │  渲染、交互
│   └─ invoke("...")  ←  @tauri-apps/api      │
├─────────────────────────────────────────────┤
│ pier-ui-tauri/src-tauri/ (Rust, Tauri 2)    │  命令桥、会话/任务状态
│   └─ 调用 pier-core::*                       │
├─────────────────────────────────────────────┤
...
| 10 | **SQLite** | HardDrive | 打开本地 `.db` 文件 | — |
...
- SSH tab 专属。
- 显示：uptime、load (1/5/15)、内存/swap、磁盘、CPU%、进程列表。
- 命令：`server_monitor_probe` 单次探测（不做自动轮询；由用户触发刷新）。
...
- 四类资源 tab：Containers / Images / Volumes / Networks。
...
- 大文件传输目前是阻塞式调用，UI 会 loading；流式进度条为未来项。

### 5.9 Log 面板

- SSH tab 专属。
- 用户输入要执行的命令（`tail -f /var/log/nginx/access.log` 等），后端用 `log_stream_start` 开启流式任务，返回 `stream_id`。
- 前端轮询 `log_stream_drain(stream_id)` 拉取 stdout/stderr/exit 事件。
- 停止：`log_stream_stop(stream_id)`。
...
- 单源 CSS 变量：`pier-ui-tauri/src/styles/tokens.css`，分 dark / light 两套。
...
- **开发**：`cd pier-ui-tauri && npm run tauri dev`
- **发布**：`cd pier-ui-tauri && npm run tauri build`
- **Cargo**：`cargo build -p pier-core` 构建纯后端。
- **版本更新**：`cd pier-ui-tauri && npm run bump <version>` 同步四处版本号（`pier-ui-tauri/package.json`、`pier-ui-tauri/src-tauri/tauri.conf.json`、`pier-ui-tauri/src-tauri/Cargo.toml`、`pier-core/Cargo.toml`）并创建 `v<version>` 标签。
...
- **Tauri 配置**（`pier-ui-tauri/src-tauri/tauri.conf.json`）：
