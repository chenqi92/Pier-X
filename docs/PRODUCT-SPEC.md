...
- **执行计划（EXPLAIN）**：查询编辑器旁的 `EXPLAIN` / `ANALYZE` 按钮包裹当前语句（不改写编辑框内容，已有的 `EXPLAIN [(...)]` 前缀去重）。`EXPLAIN` 走计划树（MySQL `FORMAT=TREE` / Postgres 文本计划 / SQLite `EXPLAIN QUERY PLAN`）；`EXPLAIN ANALYZE` 实际执行语句取真实耗时（MySQL `EXPLAIN ANALYZE`、PG `EXPLAIN (ANALYZE, BUFFERS)`）——因为会执行，**对非只读语句套用与普通写入相同的解锁 + `WRITE` 确认门禁**，只读语句无需解锁。
- **外键跳转**：表预览行展开时，按列名匹配该表外键（MySQL/PG `information_schema`、SQLite `PRAGMA foreign_key_list`，复合键合并）；点击外键行执行 `SELECT * FROM 被引用表 WHERE 被引用列 = 值 LIMIT N` 跳到被引用表。全程只读。
- **服务器活动（processlist）**：MySQL `SHOW FULL PROCESSLIST` / Postgres `pg_stat_activity` 列出会话（id / 用户 / 来源 / 库 / 命令或状态 / 时长 / 等待 / 语句），支持 全部 / 活动中 / 长耗时 过滤与刷新。每行可 **中断查询**（MySQL `KILL QUERY` / PG `pg_cancel_backend`，直接执行）或 **终止连接**（MySQL `KILL` / PG `pg_terminate_backend`，二次确认）。会话管理属于服务器运维动作，**不**受数据只读开关约束（与现有行为一致），但终止连接强制二次确认。
- **结构 Tab**：
