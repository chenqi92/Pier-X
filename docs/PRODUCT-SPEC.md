...
- **直连 TLS 模式（仅 Postgres / SQL Server）**：每条连接可选 `off` / `require` / `verify-full`（`DbAddCredentialDialog` 的 TLS 下拉，持久化到凭证模型 `DbCredential.tls_mode`，经 `useConnectionStore` 回流）。
  ...
  - **安全姿态**：TLS 只在**直连（无隧道）路径**生效——`ensureConnectionTarget` 在走 SSH 隧道 / egress forwarder（均为 127.0.0.1 回环、本已加密）时强制 `off`，仅在直拨远端主机时套用凭证选定的模式。仅 PG / SQL Server 的后端命令带 `tls_mode` 参数；MySQL / Redis / InfluxDB / Oracle / 达梦 一律明文走隧道。
  - 对话框在「直连 + 非 loopback 主机 + `off`」时给出**明文告警**，提示改用 `require` / `verify-full` 或 SSH 隧道。
...
- **直连 TLS（PG / SQL Server）**：`postgres_*`（browse / execute / list_activity / cancel_query / terminate_backend）与 `mssql_*`（execute / overview / columns）以及 `db_test_connection` 的 PG 分支接受可选 `tls_mode: Option<String>` 字符串参数，后端经 `pier_core::services::db_tls::TlsMode::from_wire` 解析（`off` / `require` / `verify-full`，未知或缺省一律 fail-safe 到 `off`）。`PostgresConfig` 走 `NoTls` / `MakeRustlsConnect`（`require` 用接受任意证书的 verifier，`verify-full` 用 webpki 根 + 主机名校验）；`SqlServerConfig` 设 tiberius 的 `EncryptionLevel` + `trust_cert`。缺省 `None` ⇒ `off` ⇒ 与历史 `NoTls` 路径字节级等价。
...
- **后端**（`pier-core/src/remote_desktop/`，UI 无关）：
  - RDP 默认经 **IronRDP**（headless，NLA/CredSSP；用户名+密码 NTLM 认证；纯 Rust，无 H.264）。**证书校验**：服务端公钥在 TLS 升级后、CredSSP 发送凭据前做 **TOFU 固定**（`remote_desktop/cert_pins.rs`，存 `rdp_known_certs.json`）——首次连接记录、后续失配阻断，复用 SSH host-key 的「信任此主机？」对话框（`ssh:host-key-prompt`），不再无条件接受任意证书。
  - RDP 可选经 **FreeRDP 3**（`rdp-freerdp` feature，默认关闭，实验中）：经 `freerdp-sys`（vendored libfreerdp3 源码构建）走 MS-RDPEGFX **H.264 / AVC420 / AVC444** 图形管道，OS 原生解码（Windows Media Foundation / macOS VideoToolbox(经 FFmpeg) / Linux OpenH264·FFmpeg），解码到 GDI BGRA/RGBX 缓冲后切脏矩形输出——与 IronRDP/VNC 同一个 `FrameSink` 接口，src-tauri 与前端不变。启用后取代 IronRDP 作为 RDP 协议实现。**证书校验**：仍为 `FreeRDP_IgnoreCertificate`，TOFU 固定（经 `VerifyCertificateEx` FFI 回调接入 `cert_pins`）待后续实现——该后端实验性、默认关闭。
