...
- 不做 Pier-X 自建 MCP 服务器接入（观察生态后再议；§5.14.8 的「M2b 受门控 agent 模式」若依赖它则一并 spike-gated 延后）
...
| **M2b 受门控 agent** | 经 stream-json 双向 control / permission 协议，或 Pier-X 侧 MCP，把 CLI 的每次工具调用回送 §5.14.4 分级器逐条审批 | 既自主又逐条门控（理想态） | 完整保留 §5.14.4 | **待 spike**（依赖 control 协议 / MCP，后者受 §5.14.7 约束，延后） |
