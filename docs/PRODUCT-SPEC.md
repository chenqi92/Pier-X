...
| 11 | **SQLite** | database | 扫描远程主机上的 `.db` / `.sqlite` 文件并打开 | 需 SSH tab |
...
- 不能触达 = 工具属于 `REMOTE_ONLY_TOOLS`（firewall / sftp / log / docker / mysql / postgres / redis / sqlite / webserver / software），且 tab 当前没有 SSH 上下文（`effectiveSshTarget` 为 null）。
