...
| 10 | **MySQL** | database | 通过 SSH tunnel 到远程 MySQL，或本地 | 需 tab |
| 11 | **PostgreSQL** | database | 同上 | 需 tab |
| 12 | **Redis** | database | 同上 | 需 tab |
| 13 | **SQLite** | database | 扫描远程主机上的 `.db` / `.sqlite` 文件并打开 | 需 SSH tab |
| 14 | **Web Server** | service | 远端 web 服务器（nginx / Apache / Caddy）一站式管理 | 需 SSH tab |
| 15 | **Software** | service | 远端工具栈一览 / 安装 / 更新 / 启用服务 | 需 SSH tab |
...
| 级 | 含义 | 行为 | 例 |
|---|---|---|---|
| **L0 只读** | 无副作用 | 自动执行；会话里渲染可展开的工具卡片（命令 + 输出全程可见） | `df -h`、`docker ps`、`SELECT`、`git status` |
| **L1 一般写** | 可恢复的写 | **逐项审批卡片**：完整命令/SQL、目标主机、风险说明；按钮 = [允许一次] [本会话允许] [总是允许（入白名单）] [拒绝] | 写文件、`git commit`、`systemctl restart`、带 WHERE 的 UPDATE |
| **L2 高危** | 难恢复 / 大范围 | 红色强确认对话框（默认焦点 = 取消）；数据销毁类还要**输入对象名**解锁（同 §5.12 卸载数据目录的设计）；**不可加白名单** | `docker rm` / `prune`、`git push --force`、DROP / TRUNCATE / 无 WHERE 的 DELETE、`FLUSHALL`、reboot、递归 chmod/chown、停 sshd |
