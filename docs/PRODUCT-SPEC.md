...
| 12 | **Nginx** | NginxIcon | 远端 nginx 配置浏览 / 结构化编辑 / 校验 / reload | 需 SSH tab |
| 13 | **Software** | Package | 远端工具栈一览 / 安装 / 更新 / 启用服务 | 需 SSH tab |

ToolStrip 第 1 项和第 2 项之间有 divider，分隔"文档/工程"与"服务"。Nginx + Software 在末尾——它们是"主机一级"的运维工具，与具体业务工具区分；Nginx 紧邻 Software 因为安装新模块（headers-more 等）会跨到 Software 面板。
...
- **版本**：仅安装/更新到发行版默认版本。版本选择器（`apt-cache madison` 等）放 v2。
...
- **SSH tab 专属**。一面板覆盖"找到配置文件 → 结构化编辑 → 校验 → reload → 软链接 toggle"全流程，不走 SFTP，不走外部编辑器。
- **位置**：右侧 ToolStrip 紧邻 Software 之前。两者都是"主机一级"运维工具——Nginx 配置编辑可能引出"装个 nginx-extras"的需求，相邻摆放方便用户串联。
- **数据源 / 命令**（全部在远端执行）：
