...
Pier-X 内置了 70+ 个常用软件的安装/探测/详情元数据。需要管理自己常用但
...
| 字段 | 必填 | 类型 | 说明 |
|---|---|---|---|
| `id` | ✓ | string | 全局唯一；与内置 bundle id（`devops` / `java-dev` / `container-ops` / `lamp` / `diagnostics` / `python-dev` / `node-dev` / `web-admin` / `monitoring` / `observability` / `netdiag` / `c-dev`）冲突时**该 bundle 被忽略** |
...
- `cleanup_scripts` — 同上游脚本一样属于内置安全边界。如果你的自定义
  ...
  而不是通过 extras 自动化。
- `provision`（引导式配置账号密码 / 原生 systemd 安装，如 MinIO）— 这类
  会触碰凭据、下载二进制、建服务的逻辑必须走代码审计，只能在内置注册表里
  声明，不开放给 extras。
