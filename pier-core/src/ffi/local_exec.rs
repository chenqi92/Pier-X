...
use crate::services::docker::{self, Container, DockerImage, DockerNetwork, DockerVolume};
...
fn to_json_value<T: Serialize>(value: &T) -> *mut c_char {
...
fn ndjson_to_json_array<T: serde::de::DeserializeOwned + Serialize>(stdout: &str) -> *mut c_char {
    let rows = docker::parse_ndjson::<T>(stdout);
    to_json_value(&rows)
}
...
pub unsafe extern "C" fn pier_local_docker_list_containers(all: i32) -> *mut c_char {
    match local_exec::docker_list_containers(all != 0) {
        Ok(s) => ndjson_to_json_array::<Container>(&s),
        Err(e) => {
            log::warn!("pier_local_docker_list_containers: {e}");
            ptr::null_mut()
        }
    }
}

/// List local images. Returns JSON array or NULL.
#[no_mangle]
pub unsafe extern "C" fn pier_local_docker_list_images() -> *mut c_char {
    match local_exec::docker_list_images() {
        Ok(s) => ndjson_to_json_array::<DockerImage>(&s),
        Err(e) => {
            log::warn!("pier_local_docker_list_images: {e}");
            ptr::null_mut()
        }
    }
}

/// List local volumes. Returns JSON array or NULL.
#[no_mangle]
pub unsafe extern "C" fn pier_local_docker_list_volumes() -> *mut c_char {
    match local_exec::docker_list_volumes() {
        Ok(s) => ndjson_to_json_array::<DockerVolume>(&s),
        Err(e) => {
            log::warn!("{e}");
            ptr::null_mut()
        }
...
/// Execute `docker <args...>` locally using a JSON array of
/// arguments. Returns
/// `{ "ok": bool, "exit_code": number, "output": string }`.
#[no_mangle]
pub unsafe extern "C" fn pier_local_docker_exec_json(args_json: *const c_char) -> *mut c_char {
    ...
    let args: Vec<String> = match serde_json::from_str(raw) {
        Ok(v) => v,
        Err(e) => {
            log::warn!("pier_local_docker_exec_json: invalid args json: {e}");
            return ptr::null_mut();
        }
    };
    match local_exec::docker_exec(&args) {
        Ok((exit_code, output)) => to_json_value(&DockerExecResponse {
            ok: exit_code == 0,
            exit_code,
            output,
        }),
        Err(e) => {
            log::warn!("pier_local_docker_exec_json: {e}");
            ptr::null_mut()
        }
    }
}
...
pub unsafe extern "C" fn pier_local_docker_action(
    ...
    if verb.is_null() || id.is_null() {
        return -1;
    }
    let v = match unsafe { CStr::from_ptr(verb) }.to_str() {
        Ok(s) => s,
        Err(_) => return -1,
    };
    let i = match unsafe { CStr::from_ptr(id) }.to_str() {
        Ok(s) => s,
        Err(_) => return -1,
    };
    match local_exec::docker_action(v, i, force != 0) {
        Ok(()) => 0,
        Err(_) => -1,
    }
}
...
pub unsafe extern "C" fn pier_local_system_metrics() -> *mut c_char {
    match local_exec::system_metrics() {
        ...
        Err(e) => {
            log::warn!("pier_local_system_metrics: {e}");
            ptr::null_mut()
        }
    }
}
...
pub unsafe extern "C" fn pier_local_exec(cmd: *const c_char) -> *mut c_char {
    ...
    let c = match unsafe { CStr::from_ptr(cmd) }.to_str() {
        Ok(s) => s,
        Err(_) => return ptr::null_mut(),
    };
