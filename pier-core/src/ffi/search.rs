...
pub unsafe extern "C" fn pier_search_files(
    ...
    if root.is_null() || pattern.is_null() { return err_json("null"); }
    let r = match unsafe { CStr::from_ptr(root) }.to_str() { Ok(s) => s, Err(_) => return err_json("utf8") };
    let p = match unsafe { CStr::from_ptr(pattern) }.to_str() { Ok(s) => s, Err(_) => return err_json("utf8") };
