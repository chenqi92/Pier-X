...
pub fn docker_list_images() -> Result<String, String> {
    ...
    if code != 0 { return Err(format!("docker images exited {code}")); }
    Ok(stdout)
}
...
pub fn docker_list_volumes() -> Result<String, String> {
    ...
    if code != 0 { return Err(format!("docker volume ls exited {code}")); }
    Ok(stdout)
}
...
pub fn docker_list_networks() -> Result<String, String> {
    ...
    if code != 0 { return Err(format!("docker network ls exited {code}")); }
    Ok(stdout)
}
...
pub fn docker_action(verb: &str, id: &str, force: bool) -> Result<(), String> {
    ...
    let (code, stdout) = exec(&cmd)?;
    if code != 0 {
        return Err(format!("docker {verb} exited {code}: {}", stdout.lines().next().unwrap_or("")));
    }
    Ok(())
}
...
pub fn docker_inspect(id: &str) -> Result<String, String> {
    ...
    if code != 0 { return Err(format!("docker inspect exited {code}")); }
    Ok(stdout)
}
...
pub fn system_metrics() -> Result<LocalMetrics, String> {
    let hostname = exec("hostname").map(|(_, s)| s.trim().to_string()).unwrap_or_default();

    let os = if cfg!(target_os = "macos") {
        exec("sw_vers -productVersion").map(|(_, s)| format!("macOS {}", s.trim())).unwrap_or_else(|_| "macOS".into())
    } else if cfg!(target_os = "linux") {
        exec("uname -r").map(|(_, s)| format!("Linux {}", s.trim())).unwrap_or_else(|_| "Linux".into())
    } else {
        "Unknown".into()
    };

    let uptime = exec("uptime").map(|(_, s)| s.trim().to_string()).unwrap_or_default();

    // Load averages
    let (load_1, load_5, load_15) = if cfg!(target_os = "macos") {
        exec("sysctl -n vm.loadavg")
            .map(|(_, s)| {
                let parts: Vec<f64> = s.trim().trim_matches(|c| c == '{' || c == '}')
                    .split_whitespace()
