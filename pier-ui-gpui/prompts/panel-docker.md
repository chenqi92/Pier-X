你在 Pier-X 仓库的一个 git worktree 里工作，分支 panel/docker，
目录 E:\workspace-freq\pier-x-docker\pier-ui-gpui。这是 Pier-X 的 GPUI（Rust）原生 UI 重写版，
基线提交 1ad739b 已经搭好面板系统。

【硬性规则】
- 只允许修改 src/panels/docker.rs 这一个文件。不要碰任何其他文件
  （panels/mod.rs、shell.rs、ui.rs、data.rs、Cargo.toml 都已接好）。
  如果你觉得需要改别的文件，说明思路错了——先停下来问。
- 你的面板已经注册好，对应工具被选中时会自动显示。它是一个 gpui View：
  pub struct DockerPanel、pub fn new(cx: &mut Context<Self>) -> Self、impl Render。
  把它们填充完整即可。
- 只用 `cargo build`（在 E:\workspace-freq\pier-x-docker\pier-ui-gpui 下）验证能编译通过。
...
- 先研究参考实现：src/shell.rs::monitor_panel（真实数据 + 1.5s 刷新循环）、
  ...
  以及 src/ui.rs（组件库）。
- 绝不能在 render 路径里阻塞。render 只负责绘制。阻塞/IO 操作（SSH 连接、列举）
...
- 提交信息风格：feat(gpui): implement docker panel，正文为客观事实条目，
  不要任何 AI/厂商署名，不要“优化/重构”这类主观词。

【本任务目标 — Docker 面板】
列出选中服务器上的容器。先渲染一个连接选择器，数据来自 data::connections_raw()；
选中后 data::connect_blocking(&cfg) 拿到 SshSession（放后台），再调
pier_core::services::docker::list_containers_blocking(&session, true)。
每个容器渲染一行：状态点（用 ui::status_dot，Container::is_running() 为真则绿色 t.pos，
否则 t.muted）、名称、镜像、端口——字段都看 pier-core/src/services/docker.rs 里的
Container 结构。把 session 和列表缓存到 View 状态。
头部用 ui::panel_header(t, "container", "DOCKER", <数量>)。
连接/列举失败用 t.neg 的一行错误提示。本期只做只读列表，不实现 启动/停止。
完成后在 E:\workspace-freq\pier-x-docker\pier-ui-gpui 下 `cargo build` 确认通过，然后提交。
