你在 Pier-X 仓库的一个 git worktree 里工作，分支 panel/firewall，
目录 E:\workspace-freq\pier-x-firewall\pier-ui-gpui。这是 Pier-X 的 GPUI（Rust）原生 UI 重写版，
基线提交 1ad739b 已经搭好面板系统。

【硬性规则】
- 只允许修改 src/panels/firewall.rs 这一个文件。不要碰任何其他文件
  （panels/mod.rs、shell.rs、ui.rs、data.rs、Cargo.toml 都已接好）。
  如果你觉得需要改别的文件，说明思路错了——先停下来问。
- 你的面板已经注册好，对应工具被选中时会自动显示。它是一个 gpui View：
  pub struct FirewallPanel、pub fn new(cx: &mut Context<Self>) -> Self、impl Render。
  把它们填充完整即可。
- 只用 `cargo build`（在 E:\workspace-freq\pier-x-firewall\pier-ui-gpui 下）验证能编译通过。
...
- 提交信息风格：feat(gpui): implement firewall panel，正文为客观事实条目，
  不要任何 AI/厂商署名，不要“优化/重构”这类主观词。

【本任务目标 — Firewall 面板】
展示选中服务器的防火墙/监听端口概览。先渲染一个连接选择器，数据来自
data::connections_raw()；选中后 data::connect_blocking(&cfg) 拿到 SshSession（放后台），
再调 pier_core::services::firewall::snapshot_blocking(&session) 得到 FirewallSnapshot
（字段看 pier-core/src/services/firewall.rs：FirewallBackend、ListeningPort、
InterfaceCounter 等）。
渲染：用 ui::section_label 分区，列出后端类型、监听端口表（端口/协议/进程，t.mono）、
接口计数等；监听中的端口用 ui::status_dot(t.pos)。可滚动。
头部用 ui::panel_header(t, "shield", "FIREWALL", <主机名或端口数>)。
连接/采集失败用 t.neg 的一行错误提示。本期只读，不做规则增删改。
完成后在 E:\workspace-freq\pier-x-firewall\pier-ui-gpui 下 `cargo build` 确认通过，然后提交。
