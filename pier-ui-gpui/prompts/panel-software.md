你在 Pier-X 仓库的一个 git worktree 里工作，分支 panel/software，
目录 E:\workspace-freq\pier-x-software\pier-ui-gpui。这是 Pier-X 的 GPUI（Rust）原生 UI 重写版，
基线提交 1ad739b 已经搭好面板系统。

【硬性规则】
- 只允许修改 src/panels/software.rs 这一个文件。不要碰任何其他文件
  （panels/mod.rs、shell.rs、ui.rs、data.rs、Cargo.toml 都已接好）。
  如果你觉得需要改别的文件，说明思路错了——先停下来问。
- 你的面板已经注册好，对应工具被选中时会自动显示。它是一个 gpui View：
  pub struct SoftwarePanel、pub fn new(cx: &mut Context<Self>) -> Self、impl Render。
  把它们填充完整即可。
- 只用 `cargo build`（在 E:\workspace-freq\pier-x-software\pier-ui-gpui 下）验证能编译通过。
...
- 提交信息风格：feat(gpui): implement software panel，正文为客观事实条目，
  不要任何 AI/厂商署名，不要“优化/重构”这类主观词。

【本任务目标 — 软件/包管理面板】
展示选中服务器的已安装软件包 / 包管理器信息。先渲染一个连接选择器，数据来自
data::connections_raw()；选中后 data::connect_blocking(&cfg) 拿到 SshSession（放后台）。
入口看 pier-core/src/services/ 下的 package_manager.rs / package_mirror.rs
（先读这些文件的公开 API，挑能在后台调用的 *_blocking 取 检测到的包管理器类型、
已安装包列表、可用更新数、当前镜像源等）。
渲染：用 ui::section_label 分区；顶部信息行（包管理器类型、包总数、可更新数）用
ui::info_row；包列表每行展示 包名 + 版本（t.mono），可用更新用 t.warn 标注。可滚动；
列表很长时可用 .overflow_y_scroll() 并考虑只渲染前 N 条 + 显示总数。
头部用 ui::panel_header(t, "package", "SOFTWARE", <包数>)。
连接/采集失败用 t.neg 的一行错误提示。本期只读，不做 安装/卸载/换源。
完成后在 E:\workspace-freq\pier-x-software\pier-ui-gpui 下 `cargo build` 确认通过，然后提交。
