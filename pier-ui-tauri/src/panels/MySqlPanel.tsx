...
export default function MySqlPanel({ tab }: Props) {
  ...
  async function browse(nextDb = dbName, nextTable = tableName) {
    ...
    try {
      const s = await cmd.mysqlBrowse({ host: host.trim(), port: p, user: user.trim(), password, database: nextDb.trim() || null, table: nextTable.trim() || null });
      setState(s); setDbName(s.databaseName); setTableName(s.tableName);
