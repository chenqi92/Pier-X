...
/**
 * Docker brand glyph (stacked containers over the whale).
 *
 * Filled rather than stroke-based, so it reads at the small sizes the
 * tool rail and list-card icons use. `fill="currentColor"` lets it
 * inherit the surrounding icon color — matches the convention the
 * lucide-react icons follow elsewhere (they use `stroke="currentColor"`
 * for the same purpose).
 *
 * API-compatible with `LucideIcon` (see `lib/rightToolMeta.ts`): accepts
 * `size` plus any standard SVG attribute. Consumers that pass
 * `strokeWidth` will be silently ignored, which is fine — this glyph
 * is not stroked.
 */
export default function DockerIcon({ size = 24, ...props }: Props) {
  return (
    <svg
